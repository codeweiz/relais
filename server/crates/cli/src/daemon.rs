...
use relais_core::session::store::SessionStore;
...
fn pid_file_path() -> Result<PathBuf> {
    Ok(Config::relais_dir()?.join("relais.pid"))
}

/// Write the current process's PID to `~/.relais/relais.pid`.
...
pub fn stop_daemon() -> Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        println!("Sent SIGTERM to Relais daemon (PID {}).", pid);
    }
...
pub fn print_status() -> Result<()> {
    if let Some(pid) = read_pid() {
        if process_alive(pid) {
            println!("Relais daemon is running (PID {}).", pid);
        } else {
            println!(
                "Relais daemon is NOT running (stale PID file for PID {}).",
                pid
            );
        }
    } else {
        println!("Relais daemon is NOT running.");
    }
    Ok(())
}
...
pub fn check_stale_pid() -> Result<bool> {
    ...
    if let Some(pid) = read_pid() {
        ...
        anyhow::bail!(
            "Relais daemon is already running (PID {}). Stop it first with `relais stop`.",
            pid
        );
    }
...
pub fn restore_sessions() -> Result<()> {
    let sessions_dir = Config::relais_dir()
        .map(|d| d.join("sessions"))
        .unwrap_or_else(|_| PathBuf::from("/tmp/relais/sessions"));
