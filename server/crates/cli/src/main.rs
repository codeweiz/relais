...
#[derive(Parser)]
#[command(name = "relais", about = "Relais Server")]
pub struct Cli {
    #[command(subcommand)]
...
#[derive(Subcommand)]
pub enum Commands {
    /// Start the Relais daemon (default when no subcommand)
    Start {
        #[arg(long)]
        port: Option<u16>,
        #[arg(long)]
...
fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        // `relais start ...` or just `relais` (no subcommand) both start the daemon.
