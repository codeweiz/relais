...
fn show_config() -> Result<()> {
    let config_path =
        relais_core::config::Config::default_path().context("could not determine config path")?;
    ...
    if config_path.exists() {
        ...
        println!();
        println!("Using defaults. Run `relais config init` to create a config file.");
    }
...
fn edit_config() -> Result<()> {
    ...
    if !config_path.exists() {
        ...
        let config = relais_core::config::Config::default();
...
fn init_config() -> Result<()> {
    ...
    if config_path.exists() {
        ...
        println!("Use `relais config edit` to modify it.");
        return Ok(());
    }
    ...
    println!();
    println!("Edit with: relais config edit");
