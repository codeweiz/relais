...
use qrcode::QrCode;
use relais_core::config::Config;
use relais_core::CoreState;
use relais_plugin_host::manager::PluginManager;
...
pub async fn start(cli: &Cli) -> anyhow::Result<()> {
    ...
    relais_server::logging::setup_logging(&config)?;

    // 3. Apply CLI overrides – top-level flags as well as Start sub-command
    //    flags are merged so that `relais --port 8080` and `relais start --port 8080`
    //    behave identically.
    ...
    // 9. Start plugin manager (discovers and spawns plugins from ~/.relais/plugins/)
    ...
    let shutdown = async move {
        ...
        if let Ok(sessions) = shutdown_core.session_store.list() {
            for mut meta in sessions {
                if meta.status == relais_core::session::types::SessionStatus::Running {
                    let new_status = match meta.session_type {
                        relais_core::session::types::SessionType::Terminal => {
                            relais_core::session::types::SessionStatus::Exited
                        }
                        relais_core::session::types::SessionType::Agent => {
                            relais_core::session::types::SessionStatus::Suspended
                        }
                    };
    ...
    let blocklist = Arc::new(relais_server::blocklist::IpBlocklist::new(Vec::new()));
    let rate_limiter = Arc::new(relais_server::rate_limit::RateLimiter::new());
            ...
            loop {
                match control_rx.recv().await {
                    Ok(event) => match event.as_ref() {
                        relais_core::events::ControlEvent::TunnelReady { url } => {
                            tracing::info!(url = %url, "tunnel URL stored");
                            *tunnel_url.write().await = Some(url.clone());
                        }
                        relais_core::events::ControlEvent::TunnelDown { .. } => {
    ...
    let app = relais_server::router::create_router(state);

    let addr = format!("{}:{}", config.server.host, config.server.port);
    let listener = tokio::net::TcpListener::bind(&addr).await?;
    tracing::info!("Relais server listening on {}", addr);
