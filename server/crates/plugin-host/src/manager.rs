...
//!
//! Discovers plugins from ~/.relais/plugins/, parses plugin.toml manifests,
...
pub struct PluginManager {
    /// Plugin directory (typically ~/.relais/plugins/).
    plugins_dir: PathBuf,
    ...
    pub async fn start_plugin(&self, manifest: PluginManifest) -> Result<(), PluginManagerError> {
        ...
        let im_bridge = if plugin_type == PluginType::Im {
            ...
            if let Some(rx) = notification_rx {
                ...
                bridge.start(rx);
                // Also listen for SessionCreated events and auto-monitor agent sessions
