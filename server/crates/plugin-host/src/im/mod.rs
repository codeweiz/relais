...
use relais_core::event_bus::EventBus;
use relais_core::events::{ControlEvent, DataEvent, SessionType};
...
impl ImBridge {
    ...
    /// Subscribe to control events and auto-monitor new agent sessions.
    ///
    /// Spawns a background task that listens for `SessionCreated` control
    /// events and auto-monitors new agent sessions so their output is forwarded
    /// to all connected IM channels without requiring an explicit `/attach`.
