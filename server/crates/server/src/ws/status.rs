...
use relais_core::events::ControlEvent;
    ...
    let json = match event {
        ControlEvent::SessionCreated {
            ...
            let st = match session_type {
                relais_core::events::SessionType::Terminal => "terminal",
                relais_core::events::SessionType::Agent => "agent",
            };
            serde_json::json!({
        ...
        ControlEvent::AgentStatusChanged { session_id, status } => {
            let status_str = match status {
                relais_core::events::AgentStatus::Initializing => "initializing",
                relais_core::events::AgentStatus::Ready => "ready",
                relais_core::events::AgentStatus::Working => "working",
                relais_core::events::AgentStatus::WaitingApproval => "waiting_approval",
                relais_core::events::AgentStatus::Idle => "idle",
                relais_core::events::AgentStatus::Crashed { .. } => "crashed",
            };
            serde_json::json!({
                "type": "agent_status",
                "session_id": session_id,
        ...
        ControlEvent::AgentError {
            ...
            let class_str = match class {
                relais_core::events::ErrorClass::Transient => "transient",
                relais_core::events::ErrorClass::Permanent => "permanent",
            };
            serde_json::json!({
                "type": "agent_error",
                "session_id": session_id,
