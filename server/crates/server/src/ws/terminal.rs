    ...
    loop {
        tokio::select! {
                    ...
                    Some(Ok(Message::Text(text))) => {
                        ...
                        match serde_json::from_str::<ClientCommand>(&text) {
                            Ok(ClientCommand::Resize { cols, rows }) => {
                                ...
                                // After the first resize, wait briefly for tmux to
                                // re-render at the new dimensions, then capture-pane.
                                if !got_initial_resize {
                                    got_initial_resize = true;
                                    // Give tmux time to re-render after resize
                ...
                match result {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Closed) => {
                        ...
                        let code = match &*status_rx.borrow() {
                            relais_core::pty::session::PtyStatus::Exited(c) => *c,
                            _ => 0,
                        };
            ...
            _ = status_rx.changed() => {
                ...
                if let relais_core::pty::session::PtyStatus::Exited(code) = status {
                    let msg = serde_json::json!({"type": "exit", "code": code});
                    let _ = ws_tx.send(Message::Text(msg.to_string().into())).await;
                    break;
                }
