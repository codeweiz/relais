...
fn test_default_config() {
    ...
    // Security defaults
    assert_eq!(cfg.security.token_file, "~/.relais/session.token");
    ...
    assert_eq!(cfg.logging.access_log, "~/.relais/logs/access.jsonl");
...
[security]
token_file = "~/.relais/session.token"
...
[logging]
level = "debug"
access_log = "~/.relais/logs/access.jsonl"
max_file_size_mb = 50
max_files = 10
...
fn test_missing_file_returns_default() {
    ...
    std::env::remove_var("RELAIS_SERVER_PORT");
    std::env::remove_var("RELAIS_SERVER_HOST");
...
fn test_tilde_expansion() {
    ...
    // token_file: ~/.relais/session.token -> /home/user/.relais/session.token
    assert!(
        cfg.security.token_file.starts_with(&*home_str),
        "token_file should start with home dir, got: {}",
        cfg.security.token_file
    );
    assert!(cfg.security.token_file.ends_with(".relais/session.token"));
...
fn test_save_and_load_roundtrip() {
    ...
    std::env::remove_var("RELAIS_SERVER_PORT");
    std::env::remove_var("RELAIS_LOGGING_LEVEL");
...
#[test]
fn test_relais_dir() {
    let relais_dir = Config::relais_dir().expect("should return relais dir");
    let home = dirs::home_dir().expect("home dir should exist");
    assert_eq!(relais_dir, home.join(".relais"));
}
