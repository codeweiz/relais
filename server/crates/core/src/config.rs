...
/// Application configuration, loaded from `~/.relais/config.toml`.
...
// ---------------------------------------------------------------------------
// Workspace config (.relais.toml) — hot-reloadable subset
// ---------------------------------------------------------------------------

/// Workspace-level configuration loaded from `.relais.toml` in a project directory.
...
impl Default for SecurityConfig {
    fn default() -> Self {
        Self {
            token_file: "~/.relais/session.token".to_string(),
...
impl Default for LoggingConfig {
    fn default() -> Self {
        Self {
            ...
            access_log: "~/.relais/logs/access.jsonl".to_string(),
            max_file_size_mb: 10,
            max_files: 5,
        }
    }
...
impl Config {
    /// Returns the path to `~/.relais/`.
    pub fn relais_dir() -> Result<PathBuf, ConfigError> {
        let home = dirs::home_dir().ok_or(ConfigError::NoHomeDir)?;
        Ok(home.join(".relais"))
    }

    /// Default config file path: `~/.relais/config.toml`.
    pub fn default_path() -> Result<PathBuf, ConfigError> {
        Ok(Self::relais_dir()?.join("config.toml"))
    }

    /// Load config from the default path (`~/.relais/config.toml`).
    ...
    /// Save config to the default path (`~/.relais/config.toml`).
    ...
    /// Migrate a v1 `config.json` (from the old Node.js RTB (legacy)) to the
    ...
    pub fn migrate_from_v1() -> Result<(), ConfigError> {
        let relais_dir = Self::relais_dir()?;
        let json_path = relais_dir.join("config.json");
        let toml_path = relais_dir.join("config.toml");
        ...
        let backup_path = relais_dir.join("config.json.v1.bak");
    ...
    /// Apply environment variable overrides with `RELAIS_` prefix.
    ///
    /// For example, `RELAIS_SERVER_PORT=8080` overrides `server.port`.
    /// Uses `_` as separator for nested keys.
    pub fn apply_env_overrides(&mut self) {
        // Server overrides
        if let Ok(v) = std::env::var("RELAIS_SERVER_HOST") {
            self.server.host = v;
        }
        if let Ok(v) = std::env::var("RELAIS_SERVER_PORT") {
            if let Ok(p) = v.parse() {
                self.server.port = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_SERVER_SHELL") {
            self.server.shell = v;
        }

        // Security overrides
        if let Ok(v) = std::env::var("RELAIS_SECURITY_TOKEN_FILE") {
            self.security.token_file = v;
        }

        // Session overrides
        if let Ok(v) = std::env::var("RELAIS_SESSION_MAX_AGE_DAYS") {
            if let Ok(p) = v.parse() {
                self.session.max_age_days = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_SESSION_MAX_STORAGE_MB") {
            if let Ok(p) = v.parse() {
                self.session.max_storage_mb = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_SESSION_SESSION_ID_LENGTH") {
            if let Ok(p) = v.parse() {
        ...
        if let Ok(v) = std::env::var("RELAIS_AGENT_AUTO_APPROVE_TOOLS") {
            if let Ok(p) = v.parse() {
                self.agent.auto_approve_tools = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_AGENT_RESTART_MAX_ATTEMPTS") {
            if let Ok(p) = v.parse() {
                self.agent.restart_max_attempts = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_AGENT_RESTART_WINDOW_SECS") {
            if let Ok(p) = v.parse() {
                self.agent.restart_window_secs = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_AGENT_RESTART_BACKOFF_BASE_SECS") {
            if let Ok(p) = v.parse() {
                self.agent.restart_backoff_base_secs = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_AGENT_RESTART_BACKOFF_MAX_SECS") {
            if let Ok(p) = v.parse() {
        ...
        // Task pool overrides
        if let Ok(v) = std::env::var("RELAIS_TASK_POOL_MAX_CONCURRENT") {
            if let Ok(p) = v.parse() {
                self.task_pool.max_concurrent = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_TASK_POOL_AUTO_APPROVE") {
            if let Ok(p) = v.parse() {
                self.task_pool.auto_approve = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_TASK_POOL_AUTO_START") {
            if let Ok(p) = v.parse() {
        ...
        // Logging overrides
        if let Ok(v) = std::env::var("RELAIS_LOGGING_LEVEL") {
            self.logging.level = v;
        }
        if let Ok(v) = std::env::var("RELAIS_LOGGING_ACCESS_LOG") {
            self.logging.access_log = v;
        }
        if let Ok(v) = std::env::var("RELAIS_LOGGING_MAX_FILE_SIZE_MB") {
            if let Ok(p) = v.parse() {
                self.logging.max_file_size_mb = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_LOGGING_MAX_FILES") {
            if let Ok(p) = v.parse() {
        ...
        // Tunnel overrides
        if let Ok(v) = std::env::var("RELAIS_TUNNEL_PROVIDER") {
            self.tunnel.provider = v;
        }
        if let Ok(v) = std::env::var("RELAIS_TUNNEL_DOMAIN") {
            self.tunnel.domain = v;
        }

        // Plugins overrides
        if let Ok(v) = std::env::var("RELAIS_PLUGINS_DIR") {
            self.plugins.dir = v;
        }
        if let Ok(v) = std::env::var("RELAIS_PLUGINS_IM_THROTTLE_INTERVAL_MS") {
            if let Ok(p) = v.parse() {
                self.plugins.im_throttle_interval_ms = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_PLUGINS_JSONRPC_TIMEOUT_SECS") {
            if let Ok(p) = v.parse() {
                self.plugins.jsonrpc_timeout_secs = p;
            }
        }
        if let Ok(v) = std::env::var("RELAIS_PLUGINS_MAX_MESSAGE_SIZE_BYTES") {
            if let Ok(p) = v.parse() {
...
/// Load workspace config from `.relais.toml` in the given directory.
