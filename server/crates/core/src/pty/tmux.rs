//! Tmux command wrappers for managing Relais terminal sessions.
//!
//! Every PTY session is backed by a tmux session named `relais-{session_id}`.
...
/// Prefix used for all Relais-managed tmux sessions.
const SESSION_PREFIX: &str = "relais-";
...
/// List all Relais-managed tmux session IDs.
///
/// Returns the extracted `session_id` portion (without the `relais-` prefix).
pub fn list_rtb_sessions() -> Result<Vec<String>> {
    ...
    debug!(count = ids.len(), "listed Relais tmux sessions");
    Ok(ids)
}

/// Kill all Relais-managed tmux sessions.
pub fn cleanup_orphans() -> Result<()> {
    let ids = list_rtb_sessions()?;
    if ids.is_empty() {
        debug!("no orphan Relais tmux sessions to clean up");
        return Ok(());
    }

    info!(count = ids.len(), "cleaning up orphan Relais tmux sessions");
    for id in &ids {
...
mod tests {
    ...
    fn test_session_name() {
        assert_eq!(session_name("abc123"), "relais-abc123");
        assert_eq!(session_name(""), "relais-");
    }
